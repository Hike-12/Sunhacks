...
class GroqCourseGenerator {
  ...
  async generateCourseStructure(inputContent, courseDetails) {
    // Check if inputContent looks like a description vs PDF content
    const isDescriptionOnly =
      inputContent.length < 1000 &&
      !inputContent.includes("Course Description:");

    if (isDescriptionOnly) {
      console.log(
        `Processing description-only course: ${inputContent.substring(
          0,
          100
        )}...`
      );
      return await this.processDescriptionOnly(inputContent, courseDetails);
    } else {
      console.log(`Processing PDF with ${inputContent.length} characters`);
      // Use existing PDF processing logic
      return await this.processComprehensivePdf(inputContent, courseDetails);
    }
  }
  ...
  async processComprehensivePdf(pdfContent, courseDetails) {
    ...
    console.log(
      `Course: ${estimatedTime} minutes → ${sectionsCount} sections → ${totalTopics} topics total`
    );
    ...
    console.log(
      `Split PDF into ${chunks.length} chunks for ${sectionsCount} sections`
    );
    ...
    for (let i = 0; i < sectionsCount; i++) {
      console.log(
        `Generating section ${
          i + 1
        }/${sectionsCount} with ${topicsPerSection} topics`
      );
      ...
      const sectionContent = chunks
        .slice(startChunk, Math.max(startChunk + 1, endChunk))
        .join("\n\n");
      ...
      try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert course designer. Create detailed educational content with exactly the requested number of topics. Always respond with valid JSON only.",
            },
            {
              role: "user",
        ...
        if (
          ...
          sectionData.children.length >= topicsPerSection
        ) {
          ...
          console.log(
            `Section ${
              i + 1
            } didn't generate enough topics, using enhanced fallback`
          );
          sections.push(
            this.createEnhancedFallbackSection(
              i + 1,
              courseDetails,
              sectionContent,
              topicsPerSection
  ...
  async processDescriptionOnly(description, courseDetails) {
    const { estimatedTime } = courseDetails;

    const sectionsCount = Math.max(4, Math.ceil((estimatedTime || 60) / 15)); // 15 minutes per section
    const topicsPerSection = 3; // Fewer topics for description-only
    const totalTopics = sectionsCount * topicsPerSection;

    console.log(
      `Description course: ${estimatedTime} minutes → ${sectionsCount} sections → ${totalTopics} topics total`
    );
    ...
    for (let i = 0; i < sectionsCount; i++) {
      console.log(
        `Generating section ${i + 1}/${sectionsCount} from description`
      );

      const sectionPrompt = this.buildDescriptionSectionPrompt(
        description,
        courseDetails,
        i + 1,
        sectionsCount,
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert course designer. Create detailed educational content from course descriptions. Always respond with valid JSON only.",
            },
            {
              role: "user",
              content: sectionPrompt,
  ...
  "children": [
    {
      ...
      "videoUrls": ["${category || "education"} tutorial", "${
      category || "learning"
    } explained"],
      "imageUrls": ["${category || "education"} concept", "${
      category || "learning"
    } diagram"],
    ...
    },
    {
      ...
      "videoUrls": ["${category || "education"} fundamentals", "${
      category || "learning"
    } basics"],
      "imageUrls": ["${category || "education"} examples", "${
      category || "study"
    } methods"],
    ...
    },
    {
      ...
      "videoUrls": ["${category || "education"} advanced", "${
      category || "learning"
    } deep dive"],
      "imageUrls": ["${category || "education"} advanced", "${
      category || "research"
    } methods"],
    ...
    },
    {
      ...
      "videoUrls": ["${category || "education"} applications", "${
      category || "learning"
    } practice"],
      "imageUrls": ["${category || "education"} practical", "${
...
`;
  ...
  buildDescriptionSectionPrompt(
    ...
    return `
Create a detailed section for an educational course based on this description:
...
Create ${topicsPerSection} educational topics that would logically fit in section ${sectionNumber} of a ${totalSections}-section course about this subject.

RESPOND WITH VALID JSON ONLY - NO MARKDOWN:

{
  "id": "section-${sectionNumber}",
  "title": "Section ${sectionNumber}: [Relevant Title for this part of the course]",
  "type": "section",
  "estimatedTime": ${sectionTime},
  "children": [
    {
      "id": "topic-${sectionNumber}-1",
      "title": "Topic 1: [First concept/skill from description]",
      "type": "topic",
      "estimatedTime": ${topicTime},
      "content": "<h3>Topic Title</h3><p>Comprehensive explanation based on the course description...</p><ul><li>Key learning point 1</li><li>Key learning point 2</li><li>Key learning point 3</li></ul><p>Detailed content that teaches this specific aspect...</p><h4>Key Takeaways:</h4><ul><li>Important concept 1</li><li>Important concept 2</li></ul>",
      "videoUrls": ["${category || "education"} ${
      title || "course"
    } tutorial", "${category || "learning"} fundamentals"],
      "imageUrls": ["${category || "education"} concepts", "${
      category || "learning"
    } examples"],
      "mermaid": "graph TD; A[Start] --> B[Learn Concept]; B --> C[Practice]; C --> D[Apply];",
      "quiz": {
        "questions": [
          {
            "question": "What is the main concept in this topic?",
            "type": "mcq",
            "options": ["Core learning objective", "Secondary concept", "Prerequisites", "Advanced topic"],
            "correctAnswer": 0,
            "explanation": "This topic focuses on the core learning objective as described."
          }
        ],
        "difficulty": "basic"
      }
    ...
    {
      "id": "topic-${sectionNumber}-2",
      "title": "Topic 2: [Second concept/skill from description]",
      "type": "topic",
      "estimatedTime": ${topicTime},
      "content": "<h3>Second Topic</h3><p>Different aspect from the course description...</p><ul><li>New learning point 1</li><li>New learning point 2</li></ul><p>Builds upon previous concepts...</p>",
      "videoUrls": ["${category || "education"} ${
      title || "course"
    } basics", "${category || "learning"} methods"],
      "imageUrls": ["${category || "education"} practices", "${
      category || "study"
    } techniques"],
      "mermaid": "graph TD; A[Previous Knowledge] --> B[New Concept]; B --> C[Understanding];",
      "quiz": {
        "questions": [
          {
            "question": "How does this topic build on previous learning?",
            "type": "mcq",
            "options": ["Extends core concepts", "Completely separate", "Only review", "Not related"],
            "correctAnswer": 0,
            "explanation": "This topic extends and builds upon the core concepts learned previously."
          }
        ],
        "difficulty": "basic"
      }
    ...
    {
      "id": "topic-${sectionNumber}-3",
      "title": "Topic 3: [Third concept/application from description]",
      "type": "topic",
      "estimatedTime": ${topicTime},
      "content": "<h3>Third Topic</h3><p>Advanced or applied concepts from the description...</p><ul><li>Application point 1</li><li>Application point 2</li></ul><p>Real-world applications and examples...</p>",
      "videoUrls": ["${category || "education"} ${
      title || "course"
    } applications", "${category || "learning"} practice"],
      "imageUrls": ["${category || "education"} real-world", "${
      category || "practical"
    } examples"],
      "mermaid": "graph TD; A[Theory] --> B[Application]; B --> C[Results];",
      "quiz": {
        "questions": [
          {
            "question": "What is the practical application of this topic?",
            "type": "mcq",
            "options": ["Real-world implementation", "Theoretical study only", "Basic introduction", "Review material"],
            "correctAnswer": 0,
            "explanation": "This topic focuses on real-world implementation and practical applications."
          }
        ],
        "difficulty": "intermediate"
      }
...
`;
  ...
  createEnhancedFallbackSection(
    ...
    for (let i = 0; i < topicsPerSection; i++) {
      const topicContent =
        contentParts[i % contentParts.length] || `Content for topic ${i + 1}`;
      const topicKeywords = this.extractKeywords(topicContent);

      topics.push({
        id: `topic-${sectionNumber}-${i + 1}`,
        title: `${
          topicKeywords || `Topic ${i + 1}`
        } - Section ${sectionNumber}`,
        type: "topic",
        estimatedTime: topicTime,
        content: `<h3>${
          topicKeywords || `Topic ${i + 1}`
        }</h3><p>Based on the content: ${topicContent.substring(
          0,
          300
        )}...</p><ul><li>Key concept ${i + 1}</li><li>Important detail ${
          i + 1
        }</li><li>Practical application ${
          i + 1
        }</li></ul><p>This topic explores the essential aspects of the subject matter and provides practical insights for better understanding.</p>`,
        videoUrls: this.enhanceVideoUrls(
          [],
          `${category || "education"} topic ${i + 1}`
        ),
        imageUrls: this.enhanceImageUrls(
          [],
          `${category || "education"} concept ${i + 1}`
        ),
          ...
          questions: [
            {
              ...
              correctAnswer: 0,
              explanation: `Topic ${i + 1} focuses on understanding ${
                topicKeywords || "the key concepts"
    ...
    try {
      ...
      if (jsonStart !== -1 && jsonEnd > jsonStart) {
        ...
        if (
          ...
          console.log(
            `Section ${sectionNumber} parsed successfully with ${parsed.children.length} topics`
          );
          return parsed;
        }
      }
  ...
  enhanceWithMultimedia(sections) {
    return sections.map((section, sectionIndex) => ({
      ...
      children: (section.children || []).map((topic, topicIndex) => ({
        ...
        content:
          topic.content ||
          `<h3>${topic.title}</h3><p>Content for this topic.</p>`,
        ...
        quiz: {
          questions:
            topic.quiz?.questions || this.generateDefaultQuiz(topic.title),
  ...
  enhanceImageUrls(imageKeywords, topicTitle) {
    ...
    const imageIds = [
      ...
      170, 180, 190, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700,
    ];
  ...
  createFallbackContent(courseDetails = {}) {
    ...
    console.log(
      `Creating fallback: ${estimatedTime} minutes → ${sectionsCount} sections`
    );
    ...
    return Array.from({ length: sectionsCount }, (_, i) =>
      this.createEnhancedFallbackSection(
        i + 1,
        courseDetails,
        "Sample educational content for comprehensive learning.",
        4
      )
    );
  }
