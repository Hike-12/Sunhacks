...
export default function PomodoroDial({ showSettings, flipped, setFlipped }) {
  ...
  const modeDurationMs = useMemo(() => {
    const mins =
      mode === "focus" ? settings.focusMinutes : settings.breakMinutes;
  ...
  const start = () => {
    ...
    setEndAt(Date.now() + (remainingMs <= 0 ? modeDurationMs : remainingMs));
    setIsRunning(true);
  };
  const pause = () => {
    setIsRunning(false);
    setEndAt(null); // Just pause, don't reset remainingMs
  };
  const reset = () => {
    setIsRunning(false);
    setEndAt(null);
  ...
  const timeStr = `${String(mm).padStart(2, "0")}:${String(ss).padStart(
    2,
    "0"
  )}`;
      ...
      <div className="w-full h-full min-h-0 bg-neutral-900/60 rounded-2xl border border-neutral-800/60 shadow-xl flex flex-col p-4 sm:p-6">
        ...
        <div className="space-y-5 w-full flex-1 overflow-y-auto">
          ...
          <div className="pt-2 flex flex-col sm:flex-row gap-2 px-0 sm:px-12">
            ...
            <button
              onClick={() => {
                if (mode === "focus")
                  setRemainingMs(settings.focusMinutes * 60 * 1000);
                else setRemainingMs(settings.breakMinutes * 60 * 1000);
              }}
              className="px-3 py-2 rounded-xl text-sm bg-indigo-400 text-black hover:bg-purple-500"
            >
              Apply now
            </button>
          </div>
          <div className="pt-4 text-xs text-neutral-500 leading-relaxed">
            Durations are saved locally. When you change them, click{" "}
            <span className="text-neutral-300">Apply now</span> to update the
            current timer.
          </div>
        </div>
        {/* Switch to Timer */}
        <div className="flex flex-col sm:flex-row gap-2 mt-6">
          <button
            ...
            className="px-4 py-2 rounded-lg bg-neutral-800 text-neutral-200 hover:bg-neutral-700 text-xs w-full sm:w-auto"
          >
            Back to Timer
          </button>
        </div>
    ...
    <div className="w-full h-full min-h-0 bg-neutral-900/60 rounded-2xl border border-neutral-800/60 shadow-xl flex flex-col items-center p-4 sm:p-6">
      ...
      <div className="w-full flex flex-col sm:flex-row items-center justify-between mb-4 gap-2">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm tracking-wide uppercase text-neutral-400">
            Pomodoro
          </span>
        </div>
      </div>
      ...
      <div className="relative w-full flex justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="text-[40px] sm:text-[56px] font-semibold tabular-nums select-none">
            {timeStr}
          </div>
      ...
      <div className="mt-6 flex flex-col items-center gap-3 w-full">
        <div className="flex flex-row justify-center gap-3 w-full">
          {!isRunning ? (
            <button
              onClick={start}
              className="px-5 py-2.5 rounded-xl bg-indigo-500 text-black font-semibold hover:bg-indigo-400 transition w-full sm:w-auto"
            >
              Start
            </button>
          ) : (
            <button
              onClick={pause}
              className="px-5 py-2.5 rounded-xl bg-indigo-400 text-black font-semibold hover:bg-purple-300 transition w-full sm:w-auto"
            >
              Pause
            </button>
          )}
          <button
            onClick={reset}
            className="px-4 py-2.5 rounded-xl bg-neutral-800 text-neutral-200 hover:bg-neutral-700 border border-neutral-700 transition w-full sm:w-auto"
          >
            Reset
          </button>
        </div>
      ...
      {/* Settings Button - Same height as controls */}
...
function Field({ label, children }) {
  return (
    <label className="block">
      <div className="mb-2 text-xs text-neutral-400 uppercase tracking-wider">
        {label}
      </div>
      {children}
    </label>
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start justify-between gap-4 p-3 rounded-xl bg-neutral-900 border border-neutral-800">
      ...
      <button
        ...
        className={
          (checked ? "bg-indigo-400" : "bg-neutral-700") +
